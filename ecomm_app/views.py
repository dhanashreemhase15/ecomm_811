...
def user_login(request):
    if request.method=='POST':
        ...
        if uname=="" or upass=="":
            ...
            u=authenticate(username=uname, password=upass)  #object row
            # print(u) # thirduser@gmail.com <-- object / none
            # print(u.is_superuser)
            if u is not None:    #value
...
def range(request):
    min=request.GET['min'] # >=500
    max=request.GET['max'] # <=5000
    # print(min)
    # print(max)
    q1=Q(is_active=True)
    q2=Q(price__gte=min)
...
def addtocart(request,pid):
    if request.user.is_authenticated:
        ...
        # print(userid)  #4
        # print(pid)    #6
        u=User.objects.filter(id=userid)   #id=4
        p=Product.objects.filter(id=pid)   #id=5
        # to check product exist or not
        q1=Q(uid=u[0])  #4
        q2=Q(pid=p[0])  #5
        c=Cart.objects.filter(q1&q2)  #queryset[<cartobject (3)>]
        n=len(c)
        context={}
        context['products']=p
        if n==1:
...
def remove(request,cid): 
    c=Cart.objects.filter(id=cid)    #id=3
    c.delete()
...
def placeorder(request):
    ...
    c=Cart.objects.filter(uid=userid)  #[object1,object2]
    # print(c)
